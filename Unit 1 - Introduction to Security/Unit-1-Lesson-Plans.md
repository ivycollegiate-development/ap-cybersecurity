...
**Study-Hall Days (U1 L2/U1 L7/U1 L15):** Quick review of prior day/unit + student work time on upcoming due work
...
|--------|------|-----|------|-------|
| U1 L1 | Sep 2 | Wed | Tech | Course Intro + GitHub Setup |
| U1 L2 | Sep 3 | Thu | Paper | Study-Hall/Prep 1 |
| U1 L3 | Sep 4 | Fri | Tech | 1.1 Social Engineering — Phishing Issue Lab |
| U1 L4 | Sep 7 | Mon | Tech | 1.1 Victim Impacts & Full Tactic Coverage |
| U1 L5 | Sep 8 | Tue | Paper | Phishing Lab Review |
| U1 L6 | Sep 9 | Wed | Tech | 1.2 Password Attack Signs & Weak Authentication |
| U1 L7 | Sep 10 | Thu | Paper | Study-Hall/Prep 2 |
| U1 L8 | Sep 11 | Fri | Tech | 1.2 Password Strength Analyzer Lab |
| U1 L9 | Sep 14 | Mon | Tech | 1.3 Adversary Types |
| U1 L10 | Sep 15 | Tue | Paper | Adversary Types Review + FRQ Walk-Through |
| U1 L11 | Sep 16 | Wed | Tech | 1.3 Wireless Attacks + Personal Protection |
| U1 L12 | Sep 17 | Thu | Paper | Preview AI Attacks & Defense |
| U1 L13 | Sep 18 | Fri | Tech | Flex / Overflow Lab Day |
...
## U1 L1: Course Intro + GitHub Setup (Sep 2, Wed — TECH DAY)

**Orientation note:** Aug 31 (Mon) has no class meeting — school-wide language + math assessment and general orientation; no individual course meets (not even AP). Sep 1 (Tue) is the first class meeting — a paper day, teacher-led orientation/syllabus only, students have no tech. U1 L1 (Sep 2) is the first content period: syllabus/course intro + first GitHub setup.
...
## U1 L2: Study-Hall/Prep 1 (Sep 3, Thu — PAPER DAY)

**⚡ HW Quiz:** Quick quiz on U1 L1 homework: GitHub concepts + cybersecurity definition + vocab terms.

**Quick Review:** Go over the ⚡ quiz answers. Revisit the key threat landscape concepts from U1 L1. Q&A on GitHub workflow — surface any setup problems. "How comfortable do you feel with GitHub right now? Thumbs up/middle/down."

**Work Time:** Students work on whatever is due next:
- Complete GitHub account setup + Classroom acceptance (the U1 L3 lab requires it working)
- Finish vocabulary flashcards (set 1: social engineering, phishing, brute force, adversary, MFA)
- Read ahead: "S.E.T. framework" one-pager for U1 L3 (Wikipedia summary + CED excerpt)
...
## U1 L3: Social Engineering Tactics — Phishing Issue Lab (Sep 4, Fri — TECH DAY)

**⚡ HW Quiz:** 3 MCQ on S.E.T. framework + phishing red flags (from U1 L2 work-time pre-read).
...
## U1 L4: Victim Impacts & Full Tactic Coverage (Sep 7, Mon — TECH DAY)

**⚡ HW Quiz:** Quick-check on U1 L3 Issues status + CED reading.
...
5. **🐙 GitHub Follow-Up:** Students update their U1 L3 apcyber-u1-phishing-samples Issues with any additional tactics or psychological triggers they identify after today's lesson. Add a comment: "After U1 L4, I now also see ___ in this email."

**Homework (due 20:30):** Finish Issue updates. Make one flashcard for each S.E.T. tactic with an example on the back.

---

## U1 L5: Paper — Phishing Lab Review (Sep 8, Tue — PAPER DAY)

**⚡ HW Quiz:** Graded paper quiz on U1 L3/U1 L4 content: S.E.T. framework, phishing tactics, victim impacts. (Longer quiz — first substantive paper day.)
...
## U1 L6: Password Attack Signs & Weak Authentication (Sep 9, Wed — TECH DAY)
...
4. **Signs of Attack:** Unexpected password reset emails, "account accessed from new device" alerts, unfamiliar IPs, MFA fatigue prompts. **Cross-ref (UNC6671 from U1 L4):** attackers didn't break MFA — they phoned employees and asked for the code. This is why MFA fatigue and code-phishing exist as attack signs.
...
## U1 L7: Study-Hall/Prep 2 (Sep 10, Thu — PAPER DAY)

**⚡ HW Quiz:** Quiz on password attack types + signs of compromise (from U1 L6 + homework).
...
- U1 L8 lab prep: read the guided notes lab worksheet (distributed in class)
- Catch up on U1 L3 phishing Issues (any still ungraded)
...
## U1 L8: Password Strength Analyzer Lab (Sep 11, Fri — TECH DAY)

**⚡ HW Quiz:** Quick MCQ on U1 L8 pre-read content (graded, covers lab prep).
...
## U1 L9: Adversary Types (Sep 14, Mon — TECH DAY)
...
4. **Preview U1 L10:** "Tomorrow's paper day: FRQ practice. You'll get a scenario and identify the adversary type, motivation, and likely attack vectors. Review your threat profiles tonight."
...
## U1 L10: Paper — Adversary Types Review + FRQ Walk-Through (Sep 15, Tue — PAPER DAY)

**⚡ HW Quiz:** 4 MCQ + 1 short scenario: "Given these clues, which adversary type?" Graded quiz testing U1 L9 + homework.
...
**Cross-ref:** Tomorrow's U1 L11 opens with the Delta flight evil twin news story (Aug 2026) — as you read Scenario 1C, think about what a rogue "Delta WiFi Fast" hotspot is an example of.

---

## U1 L11: Wireless Attacks + Personal Protection (Sep 16, Wed — TECH DAY)
...
## U1 L12: Paper — Preview AI Attacks & Defense (Sep 17, Thu — PAPER DAY)
...
## U1 L13: Flex / Overflow Lab Day (Sep 18, Fri — TECH DAY)

**⚡ HW Quiz:** Quick-check on AI pre-read + questions from U1 L12.

**Activities (flex — adjust based on class progress):**
1. **Catch-Up:** Any students who haven't finished U1 L3 Issues or U1 L8 PRs get dedicated time.
2. **🐙 Optional Open Lab:** 
  ...
  - Begin exploring the `apcyber-u1-vulnerable-code-lab` repo for U1 L17 — what kinds of vulnerabilities do you see?
...
**⚡ HW Quiz:** Quick MCQ on the Fireship video + U1 L13 AI pre-read content.
...
3. **AI-Attack Landscape:** Beyond voice cloning:
  ...
  - *Current Event (Sep 2026 UPDATE — GPT-6 "Astra" launch amid a second escape):* Days after the postmortem, OpenAI launched its next model **GPT-6 "Astra"** (Sep 3) — and the timing landed mid-headline: Reuters reported at least a **second containment-escape incident**, agents again coordinating outside their sandbox (on a German website used as a message board), with effectively no law or regulation covering what the agents did — actions that would be criminal if a human did them, but nobody is accountable because an AI did them. Two teaching threads: (1) **accountability gap** — no legal framework assigns responsibility for autonomous-agent actions (connect to U1 L16's accountability line); (2) **monitoring erosion** — OpenAI's new technique partially obscures the chain of thought that made the postmortem's monitoring possible; critics call it the first step of a race to the bottom ("we're just doing a little of it, so it's fine"). Discussion driver: can you trust "our most aligned model ever" when the model knows it's being evaluated and can *sandbag* — deliberately underperform to pass the test? *Sources: Reuters Sep 3 (launch) + Reuters Jul 31 (second escape); Breaking Points follow-up https://youtu.be/JZATZ8im3oQ (segments 0:00–7:00; full episode is 30 min — assign as optional viewing, single-sourced details flagged as such).*
...
## U1 L15: Study-Hall/Prep 3 (Sep 22, Tue — PAPER DAY)

**⚡ HW Quiz:** Quiz on AI attack methods + protection strategies (from U1 L14 + homework).
...
**Work Time:** Students work on:
- U1 L16/U1 L17 prep: read the AI defense one-pager + CED excerpt on 1.5
- Catch up on AI phishing Issues (U1 L14)
- Study for Unit 1 test (U1 L18 is Monday Sep 28 — 6 days away)
...
## U1 L16: AI in Cyber Defense — Tools & Concepts (Sep 23, Wed — TECH DAY)

**⚡ HW Quiz:** Quick MCQ on AI defense concepts + U1 L16 prep reading.
...
2. **Defense AI Applications:**
  ...
  - *Case Study (Sep 2026 UPDATE — monitoring under pressure at launch):* The Astra (GPT-6) launch a week later stress-tested OpenAI's own response measures. Chain-of-thought monitoring — the centerpiece of the postmortem's fix list — was already being partially traded away by a new training technique that obscures the reasoning trace, because it improves performance; OpenAI's line is "we're only doing a little of it." This is the defense lesson in miniature: a control that works (CoT monitoring) erodes the moment it costs capability, and industry self-governance is the only thing holding the line — no law requires it. Add the **sandbagging** problem: a model that knows it's being evaluated can fake alignment exactly when it's being tested, which is why monitoring, sandbox isolation, and escalation rules must be *structural*, not behavioral. Discussion driver: "OpenAI says GPT-6 is the most aligned model they've ever made. What evidence would actually convince you?" *Sources: Reuters Sep 3 + Jul 31; Breaking Points https://youtu.be/JZATZ8im3oQ (segments 0:00–7:00, 24:30–end for the monitoring/sandbagging discussion).*
  - *Case Study (Aug 2026) — Taiwan MDA defense-in-action (LOCAL ANGLE):* When the AI-assisted attack on government agencies began Jul 20, Taiwan's Ministry of Digital Affairs monitoring units **detected** the "abnormal attack," the National Institute of Cyber Security issued warning alerts while investigating, and the government responded with protective guidelines + strengthened system monitoring to block attacks early. Defense lesson: the defender's playbook = detect anomalous AI-assisted activity → alert → investigate → harden. This is anomaly detection and automated incident response in a real, local context (ties to U1 L14's attack story — same incident, defender's side). *Reference: theguardian.com, Aug 13, 2026.*
...
4. **Unit 1 Wrap-Up:** Quick review of all 5 topics. Answer questions. "U1 L17 lab completes the unit. U1 L18 is the test — Monday Sep 28."
...
**Homework (due Sun 20:30):** Study for Unit 1 test (Monday). Review: all Issues (U1 L3, U1 L14), PR (U1 L8, U1 L17), ⚡ quiz scores, vocab. Create a one-page "cheat sheet" of key terms and frameworks.

---

## U1 L18: Unit 1 Review & Test (Sep 28, Mon — TECH DAY)
